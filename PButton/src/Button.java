...
public class Button {
    ...
    Button(PApplet applet, String label, float x, float y, float width, float height) {
        this.applet = applet;
        this.label = label;
        this.x = x;
        this.y = y;
    ...
    void mousePressed() {
        ...
        if (areMouseCoordinatesInside(applet.mouseX, applet.mouseY)) {
    ...
    void draw() {
        if (currentState == ButtonState.Normal) {
            ...
            if (iconHover != null) {
                float iconX = x + (width - icon.width) * 0.5f;
                float iconY = y + (height - icon.height) * 0.5f;
                applet.image(iconHover, iconX, iconY);//TODO
            }
            else if (icon != null) {
                float iconX = x + (width - icon.width) * 0.5f;
                float iconY = y + (height - icon.height) * 0.5f;
            ...
            if (iconActive != null) {
                float iconX = x + (width - icon.width) * 0.5f;
                float iconY = y + (height - icon.height) * 0.5f;
                applet.image(iconActive, iconX, iconY);//TODO
            }
            else if (icon != null) {
                float iconX = x + (width - icon.width) * 0.5f;
                float iconY = y + (height - icon.height) * 0.5f;
    ...
    private boolean areMouseCoordinatesInside(int mouseX, int mouseY) {
